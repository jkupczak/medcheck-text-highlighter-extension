...
////////////
  ...
  // NOTE! - When anythings gets saved from the options page (regexes, strings),
  //         they need to be escaped first to retain the special characters in our object.
  // REF: https://stackoverflow.com/a/17863171/556079
  // Once we read them during our match script, non-regexes will get escaped AGAIN so that we are looking for the string and we ignore special characters.
  // e.g. a value of \bFoo\b entered in options.html gets entered in storage as the escaped value of \\bFoo\\b and then:
  //        - (if regex)  gets passed to our match script without modification as \\bFoo\\b
  //        - (if string) gets passed to our match script after being escaped as \\\bFoo\\\b
  // TODO Saving strings in the options menu maybe should be done with JSON.stringify - https://stackoverflow.com/a/22837870/556079

  var test = "why do I have to e\scape?";
  test = test.toString();

  stringsToFind = [
    {
      toMatch: '\\bMedBridge\\b',
      type: 'regex',
      caseSensitive: "",
      class: ''
    },
...
function createRegex(entry) {
  ...
  var toMatch = entry.toMatch;
  if ( entry.type === "string" ) {
    toMatch = entry.toMatch.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
  }

  // return the regex
  return new RegExp(toMatch, "g" + entry.caseSensitive);
}
  ...
  if ( !inIframe() ) {
    ...
    if ( totalCount > 0 ) {
      ...
      if ( !indicator.parentElement ) {
        console.log("indicator is NOT in the DOM, re-adding it");
        document.body.appendChild(indicator);
      } else {
        console.log("indicator is in the DOM already");
      }
  ...
  stringsToFind.forEach(function (entry, index) {
    ...
    findAndReplaceDOMText(document.body, {
      ...
      filterElements: function(node) {
        ...
        return node.tagName && !node.classList.contains("medcheck-text-highlight") && node.tagName !== "TEXTAREA" && node.tagName !== "SCRIPT" && node.tagName !== "STYLE";
      }
    });
  });
